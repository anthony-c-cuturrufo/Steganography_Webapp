...
def main_anonymize(model, source_path, target_path, target_cut_path):
#     parser = get_parser()
#     args = parser.parse_args()    
    anonymizer, cfg = build_anonymizer(
    ...
    assert len(image_paths) == len(image_target_paths)
    if len(image_paths) > 0:
        anonymizer.anonymize_image_paths(image_paths, image_target_paths, target_cut_path=target_cut_path)
